...
- Pipeline systemd `ipastore-scinsta-build@{env}.path` → conteneur one-shot `tools/scinsta-builder/` → IPA déposé dans `/srv/store-{env}/ipas/` → watcher lifespan crée la Version (changelog par défaut sobre : `Instagram <v> + SCInsta`, override possible via l'onglet SCInsta — clé `scinsta_meta_changelog`). **Pas d'article news auto** (l'admin publie manuellement via l'onglet News s'il le souhaite). Métadonnées App (`name`, `developer_name`, `subtitle`, `description`, `category`, `tint_color`, icône…) figées côté build — **jamais** ré-écrites aux builds suivants — mais éditables par l'admin via la section `Métadonnées` de l'onglet SCInsta (ou via l'onglet Applications une fois l'App créée). Tant que l'App n'existe pas encore, les valeurs saisies sont persistées dans les settings `scinsta_meta_*` et consommées à la création.
