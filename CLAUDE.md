...
| `.mysql.cnf` | Credentials MySQL du user `ipastore-mgmt` (utilisé par le script pour sync/reset-users) |
| `prod.env` / `dev.env` | `STORE_DIR=/srv/store-{prod\|dev}` + `IPASTORE_ENV` + `IPASTORE_BASE_URL` — variables d'environnement des conteneurs qui isolent les données entre prod et dev avec le même code. **Ne contient plus la connexion BDD**. |
| `db.json` | Config BDD (host/port/user/password/database) saisie via `/setup/database` au premier démarrage. Mode 600, owner uid 1000. |
...
- Pipeline systemd `ipastore-scinsta-build@{env}.path` → conteneur one-shot `tools/scinsta-builder/` → IPA déposé dans `/srv/store-{env}/ipas/` → watcher lifespan crée la Version (changelog par défaut sobre : `Instagram <v> + SCInsta`, override possible via l'onglet SCInsta — clé `scinsta_meta_changelog` ; si l'override correspond au template auto `Instagram X.X.X + SCInsta`, il est recalculé avec la version IG courante, pas traité comme override). **Pas d'article news auto** (l'admin publie manuellement via l'onglet News s'il le souhaite). Métadonnées App (`name`, `developer_name`, `subtitle`, `description`, `category`, `tint_color`, icône…) figées côté build — **jamais** ré-écrites aux builds suivants — mais éditables par l'admin via la section `Métadonnées` de l'onglet SCInsta (le champ `name` n'y est pas exposé, gérable depuis l'onglet Applications). Tant que l'App n'existe pas encore, les valeurs saisies sont persistées dans les settings `scinsta_meta_*` et consommées à la création.
- `bundle_id = com.burbn.instagram` ; `build_version = CFBundleVersion de l'IPA` (réel, ex: `933996394`). **Obligatoire** : SideStore compare `buildVersion` du source.json au `CFBundleVersion` de l'IPA téléchargé et refuse l'install si ça diverge. Les rebuilds de la même version IG remplacent la ligne Version en place (l'ancien IPA est supprimé). Le short SHA SCInsta reste tracé dans le setting `scinsta_last_build_scinsta_sha`.
...
- **Annulation d'un build** : bouton `Annuler le build` → `POST /scinsta/cancel` → flag `scinsta-build-cancel-<env>` → path unit `ipastore-scinsta-cancel@<env>.path` → `docker stop -t 2 scinsta-builder-<env>` (SIGTERM → 2s → SIGKILL, bloquant jusqu'à mort réelle) + écrit un result failed. `docker kill --signal=SIGTERM` ne fonctionnait pas : `build.py` est PID 1 et le kernel ignore SIGTERM sans handler pour PID 1. Conteneur builder nommé `scinsta-builder-<env>` (nom déterministe requis pour le stop).
